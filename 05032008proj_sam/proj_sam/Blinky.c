...
//	IO0DIR = LED_TRANS_PORT | LED_REC_PORT | LED_ERROR_PORT ;//|DSR_SIGNAL; //P0.16, P0.17, P0.18, P0.24 
			  									  //configured as output

	IO0SET = IO0SET | CS_RF_PORT;  //CS=1
	IO0CLR =IO0CLR | DSR_SIGNAL;  //DSR=0
			//RESET start 			   
 	IO0CLR = IO0CLR | LED_TRANS_PORT;//LED_tx=1
...
//wy:	iloùä odebranych danych, 0- prawdopodobnie nast¶pi- b-¶d w czasie odbioru pakietu
{  
...
unsigned char x;
	...
	// oczekiwanie na stan niski wyjùcia DGO0 sygnalizuj¶cy odbi°r kompletnego
	// pakietu danych
 	while ((IO0PIN & GDO_RF_PIN) !=0); 
  	...
  	{          
   		//odczyt 1 przes-anego bajtu zawieraj¶cego d-ugoùä przes-anego pakietu danych
...
{               
	...
	//oczekiwanie na stan wysoki na wyjùciu GDO -> sync transmitted
	while ((IO0PIN & GDO_RF_PIN) ==0);  
	IO0CLR = IO0CLR | LED_TRANS_PORT;//LED on
	//oczekiwanie na stan niski wyjùcia GDO -> end of packet
	while ((IO0PIN & GDO_RF_PIN) !=0);
...
//	  *pbufor -wskaËnik do tablicy z wartoùciami wpisywanymi do rejestr°w
//		ile -iloùä rejestr°w zo zapisu		
{  
unsigned char x, y, maska, wartosc;
...
//we:	adres -adres rejestru, wartosc -wpisywana wartoùä
{    
unsigned char x, maska;
	...
	//wys-anie SPI wartoùci wpisywanej do rejestru 
	maska =0x80;
	for (x=0; x<8; x++)
	{ 
		if ((wartosc &maska) ==0) IO0CLR =IO0CLR | SI_RF_PORT;//SI=0
...
unsigned char x;
  ...
  //jeùli to odpowiedË na transmisj‡ testow¶ wpisanie do transmitowanego komunikatu kodu 'R'                                                           
...
unsigned int czas_1_ramki_uart_us; 
  ...
  uart_bufor_ile_odebranych =0;//kasowanie licznika znak°w odebranych UARTEM    
  WyrownanieWskaznikowBufora();
  mnoznik_pauzy =3;
  ...
  {  
    ...
    {      
      ...
      if (uart_bufor_ile_odebranych >=(BUFOR_SIZE-10))
      {
		...
		//inicjacj‡ transmisji odebranych danych torem radiowym
		IO1SET =IO1SET | DSR_SIGNAL;//wstrzymanie transmisji nowych danych portem UART-a 
		do
		{
	...
	mnoznik_pauzy--;
	//je¨eli czas przerwy pomi‡dzy transmisj¶ kolejnych danych portem UART >2 ramek
	//zakoΩczenie odbioru UART-em i przejùcie do wysy-ania odebranych danych
	//torem radiowym
	if (mnoznik_pauzy ==0) 
	{
		Delay_us(czas_1_ramki_uart_us *2);
