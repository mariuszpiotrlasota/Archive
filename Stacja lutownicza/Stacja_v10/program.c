...
**************************************************************************************************************************/
void StanPRACA(void){
  lcd_putstr_goto("Swiatlo wlacz    ",1,1);     
   lcd_putc(0xDF); lcd_putc('C');
  
  lcd_putstr_goto("",2,1); 
  for(char go=1;go<16;go++){
    lcd_putc(0XFF);
     wait_short();
  }
...
void ShowLogoOnLCD(void){
  ...
  lcd_putstr_goto("  ST _ver: 1.0  ",1,1); 
  wait_long2();wait_long2();
